...
if($called_position == 'before_display_content' && Context::getResponseMethod()=='HTML') {
    ...
    // 쿠키를 사용하지 않음.
    if(!$addon_info->cookie_time && !$addon_info->cookie_time_select){
        $addon_info->cookie_time = -1; // 쿠키 시간(일 기준) 기본 값.
    }else if($addon_info->cookie_time_select){
        $addon_info->cookie_time = $addon_info->cookie_time_select;
    }
