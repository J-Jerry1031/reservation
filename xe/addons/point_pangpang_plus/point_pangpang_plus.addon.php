...
/**
 * @file point_pangpang_plus.addon.php
 * @brief 포인트 팡팡+
 * @nick_name 파이델(원본), 키스투엑스이(수정자)
 * 일정 확률로 포인트를 지급하는 봇을 만듭니다.
 * # GPL v2 라이센스를 적용합니다.
 **/
	...
	if($called_position == 'after_module_proc'){
		...
		if(($addon_info->is_document == 'Y' && $this->act=='procBoardInsertDocument') || ($addon_info->is_comment == 'Y' && $this->act=='procBoardInsertComment')){
			...
			$def_point = '10,50,100';
			$def_pbility = '10';
			$def_title = $addon_info->pointgirl_ment;
			$def_pang = $addon_info->comment_ment;
