<?php if(!defined("__XE__"))exit;?><!--#Meta:modules/board/skins/simple_board/spb.css--><?php $__tmp=array('modules/board/skins/simple_board/spb.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php if($__Context->module_info->use_radius == 'n'){ ?><!--#Meta:modules/board/skins/simple_board/spb_r.css--><?php $__tmp=array('modules/board/skins/simple_board/spb_r.css','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
<!--#Meta:modules/board/skins/simple_board/spb.js--><?php $__tmp=array('modules/board/skins/simple_board/spb.js','body','','');Context::loadFile($__tmp);unset($__tmp); ?>
...
<div class="xev3board">
	...
	<?php if($__Context->module_info->use_category=='Y'){ ?><ul class="cTab">
		...
		<?php if($__Context->cate_list&&count($__Context->cate_list))foreach($__Context->cate_list as $__Context->key=>$__Context->val){ ?><li<?php if($__Context->category==$__Context->val->category_srl){ ?> class="on"<?php } ?>><a href="<?php echo getUrl(category,$__Context->val->category_srl,'document_srl','') ?>"><?php echo $__Context->val->title ?><!--<?php if($__Context->val->document_count){ ?><em>[<?php echo $__Context->val->document_count ?>]</em><?php } ?>--></a>
			<?php if(count($__Context->val->children)){ ?><ul>
				<?php if($__Context->val->children&&count($__Context->val->children))foreach($__Context->val->children as $__Context->idx=>$__Context->item){ ?><li<?php if($__Context->category==$__Context->item->category_srl){ ?> class="on_"<?php } ?>><a href="<?php echo getUrl(category,$__Context->item->category_srl,'document_srl','') ?>"><?php echo $__Context->item->title ?><!--<?php if($__Context->val->document_count){ ?><em>[<?php echo $__Context->item->document_count ?>]</em><?php } ?>--></a></li><?php } ?>
			</ul><?php } ?>
		</li><?php } ?>
	</ul><?php } ?>
