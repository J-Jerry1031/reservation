...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/pointrush/tpl','header.html') ?>
...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/pointrush/tpl/pointrush_insert/1'){ ?><div class="message <?php echo $__Context->XE_VALIDATOR_MESSAGE_TYPE ?>">
	<p><?php echo $__Context->XE_VALIDATOR_MESSAGE ?></p>
</div><?php } ?>
<?php Context::addJsFile("modules/pointrush/ruleset/insertPointrush.xml", FALSE, "", 0, "body", TRUE, "") ?><form  class="x_form-horizontal" action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertPointrush" />
	<input type="hidden" name="module" value="pointrush" />
	<input type="hidden" name="act" value="procPointrushAdminInsertPointrush" />
	...
	<?php if($__Context->logged_info->is_admin != 'Y'){ ?><input type="hidden" name="pointrush_name" value="<?php echo $__Context->module_info->mid ?>" /><?php } ?>
	<input type="hidden" name="xe_validator_id" value="modules/pointrush/tpl/pointrush_insert/1" />
	<section class="section">
		<h1><?php echo $__Context->lang->subtitle_primary ?></h1>
		<?php if($__Context->logged_info->is_admin == 'Y'){ ?><div class="x_control-group">
			<label class="x_control-label" for="pointrush_name"><?php echo $__Context->lang->mid ?></label>
			<div class="x_controls">
				<input type="text" name="pointrush_name" id="pointrush_name" value="<?php echo $__Context->module_info->mid ?>" />
...
if(!$__Context->list_config[$__Context->key]){ ?><option value="<?php echo $__Context->key ?>"><?php echo $__Context->val->name ?></option><?php }} ?>
	...
	<div class="x_clearfix btnArea">
		<div class="x_pull-left">
			<a href="<?php echo getUrl('act', 'dispPointrushAdminContent') ?>" type="button" class="x_btn"><?php echo $__Context->lang->cmd_cancel ?></a>
		</div>
