...
<div class="x_clearfix">
	<div class="x_pull-left">
		별도의 저장공간으로 이동된
		<?php if($__Context->search_target){ ?> 휴면회원 중 검색 조건에 해당하는<?php } ?>
		휴면회원은 <?php echo number_format($__Context->paging->total_count) ?>명입니다.<br />
		최근 로그인순으로 표시합니다.
	</div>
...
<table id="member_expire_log" class="x_table x_table-striped x_table-hover">
	...
	<tbody>
		<?php if($__Context->moved_members&&count($__Context->moved_members))foreach($__Context->moved_members as $__Context->member){ ?><tr>
			<td class="nowr">
				<?php echo getEncodeEmailAddress($__Context->member->email_address) ?>
			</td>
			...
			<td class="nowr"><?php echo implode(', ', array_unique($__Context->moved_members_groups[$__Context->member->member_srl])) ?></td>
			<td class="nowr">
			...
			<td clas="nowr">
				<a href="#<?php echo $__Context->member->member_srl ?>" id="do_restore_<?php echo $__Context->member->member_srl ?>" class="do_restore_member" data-member-srl="<?php echo $__Context->member->member_srl ?>">복원</a>
			</td>
			<td clas="nowr">
				<a href="#<?php echo $__Context->member->member_srl ?>" id="do_delete_<?php echo $__Context->member->member_srl ?>" class="do_delete_member" data-member-srl="<?php echo $__Context->member->member_srl ?>">삭제</a>
			</td>
		</tr><?php } ?>
		<?php if(!$__Context->moved_members){ ?><tr>
			<td>해당되는 회원이 없습니다.</td>
		</tr><?php } ?>
	</tbody>
</table>
<div class="x_clearfix">
	<form class="x_pagination x_pull-left" style="margin-top:8px" action="<?php echo Context::getUrl('') ?>" method="get" ><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
