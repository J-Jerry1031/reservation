...
<div class="boardNavigation">
    <div class="buttonLeft">
        <?php if($__Context->grant->manager){ ?>
            <a href="<?php echo getUrl('','module','document','act','dispDocumentManageDocument') ?>" onclick="popopen(this.href,'manageDocument'); return false;" class="button"><span><?php echo $__Context->lang->cmd_manage_document ?></span></a>
        <?php } ?>
        <?php if($__Context->module_info->default_style != 'blog'){ ?>
        <a href="<?php echo getUrl('','mid',$__Context->mid,'page',$__Context->page,'document_srl','','listStyle',$__Context->listStyle) ?>" class="button"><span><?php echo $__Context->lang->cmd_list ?></span></a>
        <?php } ?>
    </div>
    <div class="buttonRight">
        <a href="<?php echo getUrl('act','dispBoardWrite','document_srl','') ?>" class="button green strong"><span><?php echo $__Context->lang->cmd_write ?></span></a>
    </div>
...
<form action="<?php echo getUrl() ?>" method="get" onsubmit="return procFilter(this, search)" id="fo_search" class="boardSearchForm"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
    <fieldset>
        ...
        <?php } ?>
        <span class="button"><button type="submit" onclick="xGetElementById('fo_search').submit();return false;"><?php echo $__Context->lang->cmd_search ?></button></span>
    </fieldset>
</form>
<?php } ?>
