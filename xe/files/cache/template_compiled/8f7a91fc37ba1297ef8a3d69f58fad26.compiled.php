...
<?php  $__Context->oPointModel = &getModel('point'); 
   $__Context->point = $__Context->oPointModel->getPoint($__Context->logged_info->member_srl); 
   $__Context->oModuleModel = &getModel('module');  
   $__Context->config = $__Context->oModuleModel->getModuleConfig('point'); 
   $__Context->level = $__Context->oPointModel->getLevel($__Context->point, $__Context->config->level_step);
 ?>
			...
			<?php if($__Context->is_logged){ ?><li class="item">
				...
				<div id="profile" class="tg_cnt"><button type="button" class="tg_blur"></button><button type="button" class="tg_close" title="Close this layer">X</button>
					<h2><img src="http://www.goggg.co.kr/xe/modules/point/icons/warrock_17x17/<?php echo $__Context->level ?>.gif" height="15"> <?php echo $__Context->logged_info->nick_name ?></h2>
					<ul>
...
<!--
	function chat_open(theURL) {
		var winName = 'chatopen';
		var features = 'width=450, height=670, left=0, top=0, scrollbars=no, toolbar=no, directories=no, status=no, menubar=no,  copyhistory=no, resizable=no';
                window.open(theURL, winName, features);}
//-->
</script>
<a href="javascript:chat_open('http://www.goggg.co.kr/xe/guide')">	<img width="250" height="50" alt="" src="http://www.goggg.co.kr/xe/modules/editor/styles/xeStyle/images_1/livechat.gif" border="0" /></a>
<script language="JavaScript">
<!--
	function chat_open(theURL) {
		var winName = 'chatopen';
...
<!-- // 쿠폰 -->
<!-- <a href="javascript:chat_open('http://www.goggg.co.kr/xe/index.php?module=coupon')">     <img width="250" height="50" alt="" src="http://www.goggg.co.kr/xe/modules/editor/styles/xeStyle/images_1/cupon.gif" border="0" /></a> -->
...
if($__Context->val3['link']){ ?><li><a class="lnb_3rd<?php if($__Context->val3['selected']){ ?> on<?php } ?>" href="<?php echo $__Context->val3['href'] ?>"<?php if($__Context->val3['open_window']=='Y'){ ?> target="_blank"<?php } ?>><?php echo $__Context->val3['link'] ?></a>
                ...
                <img class="zbxe_widget_output" widget="bannermgm_widget" skin="banner" colorset="default" bannermgm_srl="2949195"   img_width="250" img_height="226" link_target="_top" />
                <!-- 인포배너 -->
                <img class="zbxe_widget_output" widget="bannermgm_widget" skin="banner" colorset="default" bannermgm_srl="2930991" img_width="250" img_height="226" link_target="_top" />
                
<img class="zbxe_widget_output" widget="treasurej_popular" skin="neat_popular_tabs" tab_view="newestd,popular" option_view="image,title,content" widget_widths="250" list_count="5" module_srls="2917887,2917949,2917969,2917952,2917893,2917966" regdate="7" />
                <!-- 랭킹위젯 -->
                <img class="zbxe_widget_output" widget="phizRanking" skin="phizdefault" colorset="layout"     module_srls="2917881,2919696,2918074,2917893,2917952,2917891,2917889,2919694,2917966,2917885,2919690,2918212,2917949,2917959,2917969,2917963" period="this_week" list_count="5" without_admin="true" with_group="3,1672,884232,2884563" />
...
<div class="section"><img class="zbxe_widget_output" widget="rank_attend" skin="default" colorset="layout" title="출석부" module_srls="2896124" list_count="10" list_unit="회" without_admin="true" with_group="2,3,1672,884232,2884563" /></div>
