...
${'module_srls21_argument'} = new ConditionArgument('module_srls', $args->module_srls, 'in');
${'module_srls21_argument'}->createConditionValue();
if(!${'module_srls21_argument'}->isValid()) return ${'module_srls21_argument'}->getErrorMessage();
} else
${'module_srls21_argument'} = NULL;if(${'module_srls21_argument'} !== null) ${'module_srls21_argument'}->setColumnType('number');
...
new ConditionGroup(array(
new ConditionWithArgument('`modules`.`site_srl`',$site_srl20_argument,"equal", 'and')
,new ConditionWithArgument('`modules`.`module_srl`',$module_srls21_argument,"in", 'and')))
));
