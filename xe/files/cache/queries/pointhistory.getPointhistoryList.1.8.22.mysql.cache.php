...
$query = new Query();
$query->setQueryId("getPointhistoryList");
...
if(isset($args->htype)) {
${'htype2_argument'} = new ConditionArgument('htype', $args->htype, 'equal');
${'htype2_argument'}->createConditionValue();
if(!${'htype2_argument'}->isValid()) return ${'htype2_argument'}->getErrorMessage();
} else
${'htype2_argument'} = NULL;if(${'htype2_argument'} !== null) ${'htype2_argument'}->setColumnType('char');
...
,new ConditionWithArgument('`htype`',$htype2_argument,"equal", 'and')))
));
